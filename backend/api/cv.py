...
class CVGuideProfileRequest(BaseModel):
    profile_id: int
    job: dict


def _generate_guide(experience: str, structured_profile: dict, job: dict) -> str:
    full_job_text = ""
...
def cv_guide(req: CVGuideRequest):
    ...
    try:
        ...
        guide = _generate_guide(experience, structured_profile, req.job)
...
@router.post("/guide/profile")
def cv_guide_for_profile(req: CVGuideProfileRequest):
    if not req.job:
        raise HTTPException(status_code=400, detail="Job data is required")

    try:
        experience, structured_profile, _ = get_profile(req.profile_id)
        guide = _generate_guide(experience, structured_profile, req.job)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
